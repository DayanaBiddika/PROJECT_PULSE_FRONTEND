...
function GetSpecificProject() {
  ...
  //get the userobj from slice
  let {userObj} = useSelector(state=>state.login);
  console.log("userobj",userObj)

  //state for mentioning the indicators
  ...
  //useEffect which is a side effect
  useEffect(()=>{
  ...
  const getData = async() =>{
    ...
    //checking the role
     if(userObj.role==="GDO head"){
      //axios get request
      result=await axios.get(`http://localhost:2828/gdo-api/${state.gdoId}/portfolioDashboard/${state.projectId}`,{
        //token verification
